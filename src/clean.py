...
def clean_data(df):
    ...
    # Eliminar solo si tienen el mismo ID de factura, el mismo producto y la misma cantidad.
    df_clean = df_clean.drop_duplicates(subset=['invoice_id', 'product', 'quantity'], keep='first')
    ...
    # 2. invoice_date (Nulos iniciales): Rellenamos nulos con la transacción anterior/siguiente
